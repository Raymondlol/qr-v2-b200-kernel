...
## What this is
A GPU MODE **qr_v2** kernel competition entry: batched square compact-Householder QR matching `torch.geqrf`, on **NVIDIA B200**, ranked by **geometric mean of 12 benchmark cases**. Deadline **2026-06-30**. Confirmed official: 123203 → **7788 µs** (V4) → **5915 µs** (V5, tf32x3). `submission.py` now = V5 + **FP16x3 trailing** (branch `fp8-fp4-attack`, 2026-06-26) = **+3.6% lab geomean over V5 (est ~5700 official), 22/22, same m1.83 margin — NOT yet submitted, submit to confirm.** Leader **1292 µs**.
- **Current best = `submission.py`** (shape-routed two-level blocked compact-WY Householder; **FP16x3** trailing+gram for n≤512 [=tf32x3-class accuracy, fp16 TC=2× tf32], 1×TF32 gram+trailing for n≥1024; adaptive-ib fused Triton panel nw=8; glue-fused apply; geqrf/custom routing by shape). 22/22. Safe-margin variant: `experiments/cand_fp16x3_solvefix.py` (+2.4%, margin 800× via solve→fp32).
...
2. **Trailing GEMM precision floor = ~22 mantissa bits (tf32x3 OR FP16x3) for n≤512.** 1×TF32 FAILS n=512 **mixed at batch=640** (worst-of-640 sfr→19.7, m1.02, reseed-DQ). **CORRECTION (2026-06-26, branch `fp8-fp4-attack`, see [[qr-v2-solve-tf32-floor-artifact]] / docs/FP8_SESSION_PROGRESS.md): the documented "tf32x3 only ~2× margin" was a TF32-SOLVE artifact, NOT a trailing wall** — forcing `solve_triangular`→fp32 gives ~800× margin at +~1%. **submission.py NOW ships FP16x3 trailing** (=tf32x3-class accuracy, but fp16 TC=2× tf32 → +3.6% geomean, 22/22, same m1.83). **fp8/fp4 = accuracy-viable (m10.8 @ 6 Ozaki dots) but NOT a speed win** (K-poor batched shapes negate fp8's 4×). n≥1024 may use 1×TF32. Panel reductions must stay FP32 (low-precision panel corrupts reflectors).
