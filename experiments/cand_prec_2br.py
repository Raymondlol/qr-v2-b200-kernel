...
def _split_tf32_round(x):
    xi = x.view(torch.int32)
    hi = ((xi + (1 << 12)) & -8192).view(torch.float32)   # round-to-nearest tf32
    lo = x - hi
    return hi, lo


def _mm2br(A, B):
    Ah, _ = _split_tf32_round(A)
    Bh, Bl = _split_tf32(B)
    out = torch.matmul(Ah, Bh)
    if out.dim() == 3:
        out = torch.baddbmm(out, Ah, Bl)
...
def _mm2b(A, B):
    # 2-term: truncate A (reflector V, well-scaled), KEEP B (data C/Y) low bits.
    # = trunc(A) @ B. Error only from V's tf32 truncation (~half of 1xTF32 error).
    Ah, _ = _split_tf32(A)
    Bh, Bl = _split_tf32(B)
    out = torch.matmul(Ah, Bh)
    if out.dim() == 3:
...
def _mm2a(A, B):
    # 2-term (the cand_R one, kept for contrast): keep A, truncate B = A @ trunc(B).
    Ah, Al = _split_tf32(A)
    Bh, _ = _split_tf32(B)
    out = torch.matmul(Ah, Bh)
    if out.dim() == 3:
...
def custom_kernel(data):
    ...
    global _PREC
    _BIG_X3 = (n <= 512)
    _PREC = _PREC_N512 if n <= 512 else "1x"
    try:
        return _factor_custom(A)
    except Exception:
        return torch.geqrf(A)
