...
def _mid_block(n):
    # K-rich (recursive) blocking middle level: for n=512, NB=256 super-panel factored in
    # MB=128 mid-blocks -> the mid-level apply is K=128 (vs the old K=64 within-panel) and
    # the fat trailing is K=256 (vs K=128). Other n keep the old 2-level (MB=NB).
    if n <= 512 and n > 256:
        return 256, 128
    NB, _ = _blocking(n)
    return NB, NB
...
def _factor_panel(H, tau, n, col, b, B, ib_max, tri, stop):
    # factor sub-panels [col:stop)-bounded, applying narrow updates within [.., stop).
    j = 0
    base = col
    while base + j < stop:
        c = base + j
        m = n - c
        bb = min(_max_ib(m, ib_max), stop - c)
        if tri:
            BN = triton.next_power_of_2(m)
            BCOLS = triton.next_power_of_2(bb)
            nw = 4 if BN <= 128 else 8
            _panel_kernel[(B,)](H, tau, n, c, m, bb, BN=BN, BCOLS=BCOLS, num_warps=nw)
        else:
            _panel_factor(H[:, c:, c:c + bb], tau[:, c:c + bb])
        if c + bb < stop:                       # within-mid-block narrow update (K=bb)
            _apply_block(H, c, bb, tau, c + bb, stop)
        j += bb
...
def _factor_custom(A):
    ...
    while k < n:
        nb = min(NB, n - k)
        # factor the super-panel [k:k+nb] in MB-wide mid-blocks (mid-level apply = K=MB,
        # K-richer than the old K=ib within-panel updates)
        jm = 0
        while jm < nb:
            mcol = k + jm
            mb = min(MB, nb - jm)
            _factor_panel(H, tau, n, mcol, mb, B, ib_max, tri, mcol + mb)
            if jm + mb < nb:                     # mid-level update (K=mb)
                _apply_block(H, mcol, mb, tau, mcol + mb, k + nb)
            jm += mb
