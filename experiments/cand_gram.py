...
def _gram(A, B):
    ...
    # unconditionally the hi/lo-split _mm3; route it like the trailing instead:
    # fused tf32x3 for n<=512 (the bake-off showed fused beats the 3xcuBLAS split),
    # plain 1xTF32 for n>=1024 (loose tol; V is well-conditioned unit reflectors).
    if _BIG_X3 and _HAS_TRITON and A.is_cuda:
        return _bmm3(A, B)
    return torch.matmul(A, B)
