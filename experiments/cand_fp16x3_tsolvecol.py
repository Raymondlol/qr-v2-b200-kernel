...
if _HAS_TRITON:
    @triton.jit
    def _trsm_mT_col_kernel(Mptr, Wptr, Yptr,
                            smb, smm, smn, swb, swm, swn, syb, sym, syn,
                            BR: tl.constexpr, BLOCK_B: tl.constexpr):
        # Solve M^T @ y = w (M upper-tri) for one batch item + one RHS column per
        # program, forward substitution with y kept in registers. TRUE fp32 (no tf32),
        # so it ALSO fixes the tf32-solve margin artifact (margin ~1.9x -> ~800x) while
        # replacing the cuSOLVER batched trsm. Shape-routed (n=512, b<=128) -> legal.
        pid_b = tl.program_id(0)
        pid_n = tl.program_id(1)
        offs = tl.arange(0, BLOCK_B)
        mask = offs < BR
        w_ptrs = Wptr + pid_b * swb + offs * swm + pid_n * swn
        acc = tl.load(w_ptrs, mask=mask, other=0.0)
        for i in tl.static_range(0, BLOCK_B):
            if i < BR:
                diag = tl.load(Mptr + pid_b * smb + i * smm + i * smn)
                yi = tl.sum(tl.where(offs == i, acc, 0.0), axis=0) / diag
                coeff = tl.load(Mptr + pid_b * smb + i * smm + offs * smn,
                                mask=mask, other=0.0)
                acc = tl.where(offs == i, yi, acc)
                acc = tl.where((offs > i) & mask, acc - coeff * yi, acc)
        y_ptrs = Yptr + pid_b * syb + offs * sym + pid_n * syn
        tl.store(y_ptrs, acc, mask=mask)


def _trsm_mT_col(M, W):
    # Y = solve(M^T, W) via the custom fp32 per-column forward-sub kernel.
    Bb, b, ncols = W.shape
    Y = torch.empty_like(W)
    BB = triton.next_power_of_2(b)
    _trsm_mT_col_kernel[(Bb, ncols)](
        M, W, Y,
        M.stride(0), M.stride(1), M.stride(2),
        W.stride(0), W.stride(1), W.stride(2),
        Y.stride(0), Y.stride(1), Y.stride(2),
        BR=b, BLOCK_B=BB, num_warps=4 if BB <= 128 else 8,
    )
    return Y
...
def _apply_block(H, col, b, tau, c0, c1):
    ...
    if _HAS_TRITON and H.is_cuda and H.shape[-1] == 512 and b <= 128:
        Y = _trsm_mT_col(M, W)         # custom fp32 trsm (faster + fixes margin)
    else:
