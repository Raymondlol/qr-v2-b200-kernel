...
# Precision schedule: n<768 needs 3xTF32 (small-n tight tol / mixed-n512);
# n>=1024 passes plain 1xTF32 (tolerance grows ~20*n*eps), ~2x cheaper GEMM.
_USE_1X = False
...
def _factor_geqrf_blocked(A, block, use_1x):
    # Large-n single-stream path: cuSOLVER factors each wide panel (block),
    # trailing update on tensor cores. block large -> few cuSOLVER calls + fat GEMM.
    global _USE_1X
    _USE_1X = use_1x
    B, n, _ = A.shape
    H = A.clone()
    ...
    while k < n:
        b = min(block, n - k)
        P = H[:, k:, k:k + b]
        hp, tp = torch.geqrf(P)              # cuSOLVER panel (current stream)
        H[:, k:, k:k + b] = hp
        tau[:, k:k + b] = tp
        if k + b < n:
            _apply_block(H, k, b, tau, k + b, n)
        k += b
    return H, tau
...
def custom_kernel(data):
    ...
    if B <= 16 and n >= 2048:      # small-batch large-n: wide-block geqrf-panel + TF32
        try:
            return _factor_geqrf_blocked(A, block=512, use_1x=True)
        except Exception:
            return torch.geqrf(A)
    if _use_geqrf(B, n):
        return torch.geqrf(A)
    _USE_1X = (n >= 768)            # 1xTF32 safe for n>=1024; 3xTF32 for n<=512
    try:
        return _factor_custom(A)
    except Exception:
        return torch.geqrf(A)
