...
def _max_ib(m, ib_max):
    # Largest ib whose RESIDENT panel tile next_pow2(m)*next_pow2(ib) stays under the
    # same 48*1024-fp32-element SRAM gate the kernel respects. Mirrors the kernel's
    # BCOLS=next_power_of_2(b) rounding so it can never overflow SRAM. At the top of
    # the matrix (m=n) this returns today's ib; as m halves marching down, the byte
    # budget lets ib double for free -> fewer + fatter (K=64/128) narrow updates.
    BN = 1 << (m - 1).bit_length()
    ib = ib_max
