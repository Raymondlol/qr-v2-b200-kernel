...
def _apply_block(H, col, b, tau, c0, c1):
    ...
    M[:, idx, idx] = inv_tau
    # T = M^-1 via a SMALL b x b triangular solve (RHS=I_b), then apply T by a
    # tensor-core GEMM. Profiled Tsolve was 18-21% because the old solve had the
    # large trailing width as its RHS (cuSOLVER batched trsm ~30 TFLOP/s); moving
    # that big-RHS work to a GEMM (Y = T^T @ W) is the same trick the CholeskyQR
    # probe confirmed (trsm slow, Rinv+GEMM fast). T=0 cols: inv_tau=1e30 -> T~0.
    eye_b = torch.eye(b, device=H.device, dtype=H.dtype).unsqueeze(0).expand(M.shape[0], b, b).contiguous()
    Tfac = torch.linalg.solve_triangular(M, eye_b, upper=True)
    C = H[:, col:, c0:c1]
    W = _mm(V.transpose(1, 2), C)
    Y = _mm(Tfac.transpose(1, 2), W)
    C.sub_(_mm(V, Y))
