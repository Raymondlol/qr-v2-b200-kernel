...
def _geqrf_streamed(A):
    # small-batch large-n: overlap per-matrix cuSOLVER geqrf across CUDA streams
    B = A.shape[0]
    outs = [None] * B
    ...
    for i in range(B):
        with torch.cuda.stream(streams[i]):
            outs[i] = torch.geqrf(A[i])
    for s in streams:
        cur.wait_stream(s)
    H = torch.stack([o[0] for o in outs], 0)
    tau = torch.stack([o[1] for o in outs], 0)
    return H, tau
