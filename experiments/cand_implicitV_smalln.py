...
def _apply_block(H, col, b, tau, c0, c1):
    ...
    if c1 <= c0:
        return
    # DEEP-DIVE: route implicit-V to the LAUNCH-BOUND regime only (n<=352, 40-48%% GPU-idle).
    # n>=512 is GPU/compute-bound (1.9%% gap) where implicit-V's mask-ALU regressed officially.
    if _BIG_X3 and _HAS_TRITON and H.is_cuda and H.shape[1] <= 352:
