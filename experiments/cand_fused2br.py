...
if _HAS_TRITON:
    ...
    @triton.jit
    def _bmm_2br_kernel(A, B, C, M, N, K,
                        sab, sam, sak, sbb, sbk, sbn, scb, scm, scn,
                        BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
        ...
        for k0 in range(0, K, BK):
            ...
            ah = ((ai + 4096) & -8192).to(tl.float32, bitcast=True)   # round-to-nearest tf32 (reflector)
            bi = b.to(tl.int32, bitcast=True)
            bh = (bi & -8192).to(tl.float32, bitcast=True)            # tf32 (data)
            bl = b - bh                                               # keep data low bits
            acc += tl.dot(ah, bh, input_precision="tf32")
            acc += tl.dot(ah, bl, input_precision="tf32")
            ap += BK * sak; bp += BK * sbk
...
def _bmm2br(A, B):
    ...
    _bmm_2br_kernel[grid](A, B, C, M, N, K, *A.stride(), *B.stride(), *C.stride())
    return C
