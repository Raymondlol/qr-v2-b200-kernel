...
if __name__ == "__main__":
    import traceback
    print(f"=== M3c-0 tcgen05 tf32x3 apply (per-matrix) vs torch.geqrf  NB={NB} ===")
    torch.manual_seed(0)
    for B, n in [(1, 32), (1, 64), (1, 128), (4, 128), (4, 256), (1, 100), (2, 512)]:
        ...
        try:
            ...
            _run_cute(H, tau, _scratch(B, "cuda")); torch.cuda.synchronize()
    ...
    print("\n=== TIMING n=512 (cute M3c-0 vs torch.geqrf), median us ===")
    print("  (ref: V9=5791us, V10=5343us official @ full 12-case geomean; this is single-shape Modal)")
    for B in [64, 256, 640]:
        try:
            ...
            t_cute = _time(lambda: _run_cute(H, tau, scr))
            t_geqrf = _time(lambda: torch.geqrf(A))
            print(f"  B={B:4d} n=512: cute={t_cute:10.1f}us  geqrf={t_geqrf:9.1f}us  ratio={t_cute/t_geqrf:.2f}x geqrf")
        except Exception:
            print(f"  B={B} n=512 timing: EXC\n" + traceback.format_exc()[-1500:])
    print("DONE")
